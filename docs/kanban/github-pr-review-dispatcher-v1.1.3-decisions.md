# GitHub PR Review Dispatcher v1.1.3 — immutable release identity, CI producer provenance, adapter and rollout decision

Status: design complete; implementation and activation remain disabled pending independent review.

## Decision

Adopt `github-pr-review-dispatcher/v1.1.3` as the sole dispatcher contract and publish `github-hermes-development-workflow` v5.1.2 as its operator/agent workflow. The contract owns the machine protocol; the skill explains how a development agent uses it. In conflicts, the contract wins. This is a direct-parent, fail-closed reconstruction on immutable v1.1.1 commit `2e6999f945836b4fe43e5127d3068222e297c6eb`; its ref is historical and remains unmoved. The semantically accepted v1.1.2 controls are unchanged; only version and immutable release identity change. The exact publication repository is `Mateo817/hermes-agent` and the exact create-only immutable ref is `refs/heads/contracts/github-pr-review-dispatcher-v1.1.3-scope3-from-2e6999f`.
...
## Resolved v5-to-v1.1.3 discrepancies

| Topic | Workflow-v5/v1.1.1 statement or ambiguity | v1.1.3 resolution |
|---|---|---|
...
6. Publish a release manifest binding the full commit SHA, durable non-default ref `refs/heads/contracts/github-pr-review-dispatcher-v1.1.3-scope3-from-2e6999f` in publication repository `Mateo817/hermes-agent`, contract path/blob SHA-256, skill path/blob SHA-256 and this v1.1.3 decision-note path/blob SHA-256.
...
- `SKILL_SHA256`: SHA-256 of `docs/kanban/github-hermes-development-workflow-SKILL-v5.1.md` version `5.1.2` at that commit.
...
`skills inspect` is immutable-URL preview and trust display only; `Trust: community` is expected and is neither PASS nor a security scan. The actual no-force `skills install` performs quarantine/security scanning. Any scan or quarantine rejection stops `BLOCKED`; it must not be bypassed with `--force`. After success, read the installed file bytes from the exact profile's resolved skill home, recompute SHA-256, and verify `github-hermes-development-workflow` version `5.1.2` enabled for that profile; command success or list output alone is insufficient. Verify no unrelated profile/skill file changed. Delete the temporary download after verification. Because session prompt state is cached, installation takes effect in a new/reset session; the CLI install subcommand has no `--now` option. Rollback for a newly installed copy is `hermes -p "$TARGET_PROFILE" skills uninstall github-hermes-development-workflow --yes`, followed by the same listing/readback; a pre-existing version is restored from the pre-write backup using supported tooling rather than overwritten heuristically.
